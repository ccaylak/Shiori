                ...
                VStack(alignment: .leading) {
                    ...
                    ScrollView(.horizontal, showsIndicators: false) {
                        LazyHStack(spacing: 10) {
                            ForEach(details?.data.voices ?? [] , id: \.anime.malId) { voiceactor in
                                NavigationLink(destination: CharacterDetailsView(characterData: MetaData(malId: voiceactor.anime.malId, name: voiceactor.anime.title ?? "", images: JikanImages()), role: voiceactor.role)) {
                                    VStack {
                                        AsyncImageView(imageUrl: voiceactor.anime.images.jpgImage.baseImage)
                                            .frame(width: CoverSize.medium.size.width, height: CoverSize.medium.size.height)
                                            .cornerRadius(12)
                                            .strokedBorder()
                                        Text(voiceactor.anime.title ?? "")
                                            .font(.caption)
